    ...
    setTimeout(() => {
      this.storage.get('ID').then((val) => {
        ...
        this.storage.get('Name').then((val) => {
          this.Name=val;
          this.http.get('http://localhost:5000/requestDelivery', { params: { 'PackageID': PackageID, 'PackageName': PackageName, 'TransporterID': this.ID, 'SenderID': SenderID,'TransporterName':this.Name } }).map(res => res.json()).subscribe(response => {
