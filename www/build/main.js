...
var DeliveredPage = (function () {
        ...
        this.storage.get('ID').then(function (val) {
            ...
            _this.http.get('http://localhost:5000/deliveredPackges', { params: { 'TransporterID': _this.ID, 'skips': _this.skips } })
                ...
                if (response.content == "failed") {
                    _this.responseDataDelivered = []; //set to empty
                    _this.presentAlert("No Delivered Packages Found"); //prompt error to user
                }
                else {
                    response.content.map(function (item) {
    ...
    DeliveredPage.prototype.presentAlert = function (text) {
        var alert = this.alertCtrl.create({
            title: text,
            buttons: ['Dismiss'],
        });
        ...
        if (/^\d{8}$/.test(this.token)) {
            ...
            this.http.get('http://localhost:5000/deliveryCompleted', { params: { 'PackageID': PackageID, 'token': this.token } }).map(function (res) { return res.json(); }).subscribe(function (response) {
                if (response.content == 'success') {
                    ...
                    loading.dismiss();
                    _this.presentNotification("Verification Key Mismatch", "Failed"); //wrong token was entered
                }
            }, function (err) {
                console.log('error');
            });
...
var ProfilePage = (function () {
    ...
    ProfilePage.prototype.getData = function () {
        ...
        return new Promise(function (resolve, reject) {
            ...
            _this.storage.get('ID').then(function (val) {
                _this.ID = val;
                console.log("prof" + _this.profileImage);
                console.log("ID" + _this.ID);
                _this.http.get('http://localhost:5000/gettransporterdata', { params: { 'TransporterID': _this.ID } }).map(function (res) { return res.json(); }).subscribe(function (response) {
                    console.log(response.content);
                    _this.name = response.content[0].Name;
                    //console.log(this.name);
                    _this.contantInfo = response.content[0].Phone;
                    //console.log(this.contantInfo);
                    _this.rating = Array(Math.round(response.content[0].Rating)).fill(Math.round(response.content[0].Rating));
                    ...
                    _this.DeliveredPackages = response.content[0].DeliveredPackages;
                    if (_this.rating != 0) {
...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_10__pages_enroute_enroute__ = __webpack_require__(152);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_11__pages_list_list__ = __webpack_require__(460);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_12__pages_nearby_nearby__ = __webpack_require__(155);
...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_25__pages_help_help__ = __webpack_require__(151);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_26__pages_login_login__ = __webpack_require__(153);
...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_11__pages_help_help__ = __webpack_require__(151);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_12__pages_login_login__ = __webpack_require__(153);
...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_17_rxjs_Observable__ = __webpack_require__(9);
...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_18_rxjs_add_observable_interval__ = __webpack_require__(456);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_18_rxjs_add_observable_interval___default = __webpack_require__.n(__WEBPACK_IMPORTED_MODULE_18_rxjs_add_observable_interval__);
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
...
var MyApp = (function () {
    function MyApp(platform, statusBar, splashScreen, events, storage, fcm, alertCtrl, http, geolocation) {
        ...
        this.http = http;
        this.geolocation = geolocation;
        this.NotificationData = []; //to store notification data temporar ily
    ...
    MyApp.prototype.subscribeWatch = function () {
        //this.watch = this.geolocation.watchPosition();
        ...
        // });
        // this.watch.unsubscribe();
        this.observer = __WEBPACK_IMPORTED_MODULE_17_rxjs_Observable__["Observable"].interval(5000).subscribe(function () {
            _this.geolocation.getCurrentPosition().then(function (position) {
                console.log("ALoha" + position.coords.latitude, position.coords.longitude);
            ...
            if (data.wasTapped) {
                if (data.PackageAcceptance == "true") {
                    _this.storage.get('NotificationData').then(function (val) {
                        _this.string = "Your Bid for" + data.PackageName + "has been accepted";
                        if (val != null) {
                            _this.NotificationData = val;
                        }
                    ...
                    _this.string = "Your Bid for" + data.PackageName + "has been denied";
                    console.log(_this.string);
                    _this.showNotification(_this.string);
                    _this.NotificationData.push(_this.string);
                    _this.storage.set('NotificationData', _this.NotificationData); //notification data
                    _this.string = "";
                }
    ...
    PackagedetailPage.prototype.sendRequest = function (PackageID, PackageName, SenderID) {
        ...
        setTimeout(function () {
            _this.storage.get('ID').then(function (val) {
                ...
                _this.storage.get('Name').then(function (val) {
                    _this.Name = val;
                    _this.http.get('http://localhost:5000/requestDelivery', { params: { 'PackageID': PackageID, 'PackageName': PackageName, 'TransporterID': _this.ID, 'SenderID': SenderID, 'TransporterName': _this.Name } }).map(function (res) { return res.json(); }).subscribe(function (response) {
                        if (response.content == 'success') {
...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_0__angular_core__ = __webpack_require__(0);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_1_ionic_angular__ = __webpack_require__(7);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_2__enroute_enroute__ = __webpack_require__(152);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_3__nearby_nearby__ = __webpack_require__(155);
